  ...
  <div class="flex flex-col container-size rounded-xl bg-[var(--ui-bg)] shadow-lg p-4">
    ...
    <div class="flex flex-col items-center justify-center h-full gap-4 pb-8 w-[80%] mx-auto">
      <div class="flex items-center gap-2">
        ...
        <img :src="chainMap[useChain.chain.id]?.icon" class="w-5 h-5" :title="useChain.chain.name" />
      </div>
      ...
      <div v-if="step === 1" class="w-full">
        <UForm :state="formState" @submit="onSubmit" class="w-full">
          ...
          <!-- Mainnet deploy + gas warning -->
          <div v-if="showMainnetDeployWarning" class="mt-3 p-3 bg-amber-50 rounded-lg border border-amber-200">
            <p class="text-xs text-amber-700 leading-relaxed">
              {{ i18n.text['transfer.mainnetDeployWarning'] || '首次在主网转账将同时部署 Safe 合约。主网无 Gas 代付，部署 + 转账的 Gas 费将从你的钱包余额中扣除。请确保有足够 ETH。' }}
            </p>
          </div>
...
const safeNotDeployed = ref(false);

const showMainnetDeployWarning = computed(() =>
  useChain.chain.id === 1 && safeNotDeployed.value
);
...
const handleError = (error: unknown, title: string, description?: string) => {
  ...
  toast.add({
    title,
    description:
      description || (error instanceof Error ? error.message : i18n.text["Please try again later"]),
    color: "error",
  });
};
  ...
  try {
    if (isPhoneNumber(formState.to)) {
      ...
      if (!recipient?.evm_chain_address) {
        handleError(
          new Error(i18n.text["Invalid phone number"]),
          i18n.text["Invalid phone number"]
        );
        return false;
      }
    ...
    handleError(
      error,
      i18n.text["Get recipient address failed"],
...
const fetchGasCredits = async (): Promise<boolean> => {
  try {
    ...
    handleError(
      error,
      error instanceof Error ? error.message : i18n.text["Get free transaction count failed"],
      i18n.text["Please try again later"]
    );
    return false;
  }
};
...
const initForm = async () => {
  ...
  if (chain_id && typeof chain_id === "string") {
    ...
    if (numChainId in chainMap) {
      ...
      handleError(
        ...
        i18n.text["Invalid chain ID"],
        i18n.text["Chain ID in URL is incorrect"]
      );
    }
  }
  ...
  if (token_address && typeof token_address === "string") {
    ...
    if (!targetToken) {
      handleError(
        new Error(i18n.text["Invalid token address"]),
        i18n.text["Invalid token address"],
        i18n.text["Token address in URL is incorrect or unsupported token"]
      );
    }
  } else {
  ...
  if (amount && typeof amount === "string") {
    ...
    if (!isNaN(numAmount) && numAmount > 0) {
      ...
      handleError(
        new Error(i18n.text["Invalid amount"]),
        i18n.text["Invalid amount"],
  ...
  // Check if Safe is deployed on current chain (for mainnet deploy warning)
  try {
    const addr = await getActiveSmartAccountAddress();
    if (addr) {
      const { getBytecode } = await import("viem");
      const client = semiCore().chain(useChain.chain.id).publicClient;
      const code = await getBytecode(client, { address: addr });
      safeNotDeployed.value = !code || code === "0x";
    }
  } catch {
    // If check fails, don't show warning
    safeNotDeployed.value = false;
  }
};
...
const handleQrCodeDetect = (values: string[]) => {
  if (values.length > 0) {
    ...
    if (isAddress(address)) {
      ...
      handleError(
        new Error(i18n.text["Invalid address"]),
        i18n.text["Invalid address"],
...
// Refresh balance on chain switch (activeWallet getter changes with chain even if activeWalletId doesn't)
watch(
  () => useChain.chain.id,
  async () => {
    fetchTokenBalance();
    // Re-check Safe deployment status on the new chain
    try {
      const addr = await getActiveSmartAccountAddress();
      if (addr) {
        const { getBytecode } = await import("viem");
