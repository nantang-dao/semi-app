...
import { type EthereumProvider, toOwner } from "./toOwner";
...
function isErc7579Args<entryPointVersion extends "0.6" | "0.7" = "0.7">(
  args:
    | ToSafeSmartAccountParameters<entryPointVersion, Address | undefined>
    | PredictSafeSmartAccountAddressParameters<entryPointVersion, Address | undefined>
): args is
  | ToSafeSmartAccountParameters<entryPointVersion, Address>
  | PredictSafeSmartAccountAddressParameters<entryPointVersion, Address> {
      ...
      if (to === multiSendCallOnlyAddress) {
        ...
        while (position < dataLength) {
          ...
          const dataLength = Number(
            BigInt(slice(dataToDecode, position, position + 32)) * BigInt(2)
          );
...
export type PredictSafeSmartAccountAddressParameters<
  entryPointVersion extends "0.6" | "0.7",
  TErc7579 extends Address | undefined,
> = {
  client: Client<Transport, Chain | undefined, JsonRpcAccount | LocalAccount | undefined>;
...
export async function predictSafeSmartAccountAddress<
  entryPointVersion extends "0.6" | "0.7",
  TErc7579 extends Address | undefined,
>(
  parameters: PredictSafeSmartAccountAddressParameters<entryPointVersion, TErc7579>
): Promise<Address> {
  const {
    client,
    owners: _owners,
